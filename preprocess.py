...
class Indexer:
    def __init__(self, symbols = ["<blank>","<unk>","<s>","</s>"]):
        ...
        self.BOS = symbols[2]
        self.EOS = symbols[3]
        self.d = {self.PAD: 0, self.UNK: 1, self.BOS: 2, self.EOS: 3} # change pad -> 0

    def add_w(self, ws):
        for w in ws:
            if w not in self.d:
                self.d[w] = len(self.d) # change from len(self.d)+1 -> len(self.d)
    ...
    def prune_vocab(self, k):
        ...
        for word in self.pruned_vocab:
            if word not in self.d:
                self.d[word] = len(self.d)  # change from len(self.d)+1 -> len(self.d)
